				...
				<div class="nav-tabs-custom">
					...
					<div class="tab-content">
						...
						<div class="tab-pane" id="tab-custom-link">
							...
							{!! Form::open(['action' => '\Lehungdev\Cms\Controllers\MenuController@store', 'id' => 'menu-custom-form']) !!}
...
<div class="modal fade" id="EditModal" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			{!! Form::open(['action' => ['\Lehungdev\Cms\Controllers\MenuController@update', 1], 'id' => 'menu-edit-form']) !!}
...
$(function () {
	...
	$('#menu-nestable').on('change', function() {
		...
		$.ajax({
			url: "{{ url(config('cms.adminRoute') . '/la_menus/update_hierarchy') }}",
			method: 'POST',
			data: {
				jsonData: jsonData,
				"_token": '{{ csrf_token() }}'
	...
	$("#tab-modules .addModuleMenu").on("click", function() {
		...
		$.ajax({
			url: "{{ url(config('cms.adminRoute') . '/la_menus') }}",
			method: 'POST',
			data: {
				type: 'module',
				module_id: module_id,
